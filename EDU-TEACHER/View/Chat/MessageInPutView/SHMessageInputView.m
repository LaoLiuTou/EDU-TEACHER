...
- (UITextView *)textView{
    if (!_textView) {
        ...
        _textView.frame = CGRectMake(2*kSHInPutSpace + kSHInPutIcon_WH, self.height - kSHInPutIcon_WH - kSHInPutSpace, self.emojiBtn.x - self.changeBtn.maxX - 2*kSHInPutSpace+kSHInPutIcon_WH, kSHInPutIcon_WH);
...
- (UIButton *)emojiBtn{
    if (!_emojiBtn) {
        ...
        _emojiBtn.frame = CGRectMake(self.width - 2*(kSHInPutIcon_WH + kSHInPutSpace), self.changeBtn.y, 0, 0);
...
- (void)sendMessageWithImage:(UIImage *)image{
    ...
    if ([_delegate respondsToSelector:@selector(chatMessageWithSendImage:imageName:size:)]) {
        [_delegate chatMessageWithSendImage:image imageName:imageName size:image.size];
    }
}
