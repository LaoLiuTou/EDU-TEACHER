...
@interface AddDormSignInView ()<UITextViewDelegate>

@end

@implementation AddDormSignInView



-(instancetype)initWithFrame:(CGRect)frame{
    if (self=[super initWithFrame:frame]) {
        self.frame = CGRectMake(0, 0, kWidth, 570);
...
- (int)initView{
    ...
    //仅一次
    UIButton *timesBack=[[UIButton alloc] init];
    timesBack.backgroundColor=[UIColor whiteColor];
    timesBack.tag=200;
    [self addSubview:timesBack];
    [timesBack mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(sign_deviceback.mas_bottom).offset(6);
        make.width.mas_equalTo(kWidth);
        make.height.mas_equalTo(50.0f);
    }];
    UILabel *timesTitle=[[UILabel alloc] init];
    timesTitle.textColor=[UIColor blackColor];
    timesTitle.font=[UIFont systemFontOfSize:14];
    timesTitle.text=@"仅一次";
    [timesBack addSubview:timesTitle];
    [timesTitle mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(timesBack).offset(15);
        make.centerY.equalTo(timesBack);
        make.width.mas_equalTo(100);
        make.height.mas_equalTo(50.0f);
    }];
    UISwitch *timesSwitch=[[UISwitch alloc] init];
    _timesSwitch=timesSwitch;
    [_timesSwitch setBackgroundColor:GKColorRGB(246, 246, 246)];
    [_timesSwitch setOnTintColor:GKColorRGB(246, 246, 246)];
    [_timesSwitch setThumbTintColor:GKColorHEX(0x2c92f5, 1)];
    _timesSwitch.on=YES;
    _timesSwitch.layer.cornerRadius = 15.5f;
    _timesSwitch.layer.masksToBounds = YES;
    [_timesSwitch addTarget:self action:@selector(timesSwitchAction:) forControlEvents:UIControlEventValueChanged];
    
    [timesBack addSubview:_timesSwitch];
    [_timesSwitch mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(timesBack.mas_right).offset(-15);
        make.centerY.equalTo(timesBack);
    }];
    ...
    [_timesView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(timesBack.mas_bottom).offset(1);
        make.width.mas_equalTo(kWidth);
        make.height.mas_equalTo(0);//0,101
    }];
    
    //重复时间
    UIButton *resigninBack=[[UIButton alloc] init];
    resigninBack.backgroundColor=[UIColor whiteColor];
    [resigninBack addTarget:self action:@selector(clickResignin) forControlEvents:UIControlEventTouchUpInside];
    [_timesView addSubview:resigninBack];
    [resigninBack mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.timesView);
        make.width.mas_equalTo(kWidth);
        make.height.mas_equalTo(50);
    }];
    UILabel *resigninTitle=[[UILabel alloc] init];
    resigninTitle.text=@"重复时间";
    resigninTitle.font=[UIFont systemFontOfSize:14];
    resigninTitle.textColor=[UIColor blackColor];
    resigninTitle.textAlignment=NSTextAlignmentLeft;
    [resigninBack addSubview:resigninTitle];
    [resigninTitle mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(resigninBack).offset(15);
        make.centerY.equalTo(resigninBack);
        make.width.mas_equalTo(kWidth/2);
        make.height.mas_equalTo(50);
    }];
    //选择结果
    UIScrollView *resigninScrollView=[[UIScrollView alloc] init];
    _resigninScrollView=resigninScrollView;
    _resigninScrollView.showsHorizontalScrollIndicator = NO;
    _resigninScrollView.backgroundColor=[UIColor whiteColor];
    _resigninScrollView.bounces = YES;
    [resigninBack addSubview:_resigninScrollView];
    UITapGestureRecognizer *selectResignin= [[UITapGestureRecognizer alloc] initWithTarget:self  action:@selector(clickResignin)];
    [_resigninScrollView addGestureRecognizer:selectResignin];
    [_resigninScrollView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(resigninBack).offset(115);
        make.centerY.equalTo(resigninBack);
        make.width.mas_equalTo(kWidth-145);
        make.height.mas_equalTo(50);
    }];
   
    UIImageView *resigninImage=[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"jiahao"]];
    _resigninImage=resigninImage;
    [resigninBack addSubview:_resigninImage];
    [_resigninImage mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(resigninBack).offset(-30);
        make.centerY.equalTo(resigninBack);
        make.width.mas_equalTo(20);
        make.height.mas_equalTo(20);
    }];
    UIImageView *resigninRightImage=[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"youjiantou"]];
    [resigninBack addSubview:resigninRightImage];
    [resigninRightImage mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(resigninBack).offset(-10);
        make.centerY.equalTo(resigninBack);
        make.width.mas_equalTo(24);
        make.height.mas_equalTo(24);
    }];
    ...
    [_timesView addSubview:stoptimeBack];
    [stoptimeBack mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(resigninBack.mas_bottom).offset(1);
        make.width.mas_equalTo(kWidth);
        make.height.mas_equalTo(50.0f);
    }];
    ...
    [selectback mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(sign_end_timeBack.mas_bottom).offset(1);
        make.width.mas_equalTo(kWidth);
        make.height.mas_equalTo(50);
    }];
    ...
    [commentback mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(selectback.mas_bottom).offset(6);
        make.width.mas_equalTo(kWidth);
...
- (void) timesSwitchAction:(UISwitch *)timesSwitch { 
    if ([timesSwitch isOn]){
        ...
        [_timesSwitch setThumbTintColor:[UIColor grayColor]];
        [_timesView mas_updateConstraints:^(MASConstraintMaker *make) {
    ...
    if([self.delegate respondsToSelector:@selector(timesSwitchAction:)]){
        [self.delegate timesSwitchAction:timesSwitch];
    }
}
...
#pragma mark - 重复时间
-(void)clickResignin {
    if([self.delegate respondsToSelector:@selector(clickResignin)]){
        [self.delegate clickResignin];
    }
}
#pragma mark - 时间
