...
@property (nonatomic, copy) UITextField *nickNameLabel;
 ...
 */
+ (void)showUpdateAlertWithVersion:(NSString *)version Descriptions:(NSArray *)descriptions Image:(UIImage *) image
{
    ...
    NSLog(@"====%@",description);
    SELUpdateAlert *updateAlert = [[SELUpdateAlert alloc]initVersion:version description:description Image:image];
 ...
 */
+ (void)showUpdateAlertWithVersion:(NSString *)version Description:(NSString *)description Image:(UIImage *) image
{
    SELUpdateAlert *updateAlert = [[SELUpdateAlert alloc]initVersion:version description:description Image:image];
    [[UIApplication sharedApplication].delegate.window addSubview:updateAlert];
}

- (instancetype)initVersion:(NSString *)version description:(NSString *)description Image:(UIImage *) image
{
    self = [super init];
    if (self) {
...
- (void)_setupUI
{
    ...
    //bgView实际高度
    CGFloat realHeight = descHeight + Ratio(378);
    ...
    if (realHeight > DEFAULT_MAX_HEIGHT) {
        scrollEnabled = YES;
        descHeight = DEFAULT_MAX_HEIGHT - Ratio(378);
    }else
    {
    ...
    UIImageView *updateIcon = [[UIImageView alloc]initWithFrame:CGRectMake((updateView.frame.size.width - Ratio(260))/2, Ratio(20), Ratio(260), Ratio(260))];
    //updateIcon.image = [UIImage imageNamed:@"VersionUpdate_Icon"];
    updateIcon.image = self.qrcodeImage;
    ...
    versionLabel.text = [NSString stringWithFormat:@"%@",self.version];
    [updateView addSubview:versionLabel];
    
    //更新内容
    UITextView *descTextView = [[UITextView alloc]initWithFrame:CGRectMake(Ratio(28), Ratio(10) + CGRectGetMaxY(updateIcon.frame), updateView.frame.size.width - Ratio(56), descHeight)];
    ...
    [updateButton setTitle:@"保存到本地" forState:UIControlStateNormal];
...
//    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:str]];
  ...
  //[self dismissAlert];
    UIImageWriteToSavedPhotosAlbum(self.qrcodeImage, self, @selector(image:didFinishSavingWithError:contextInfo:), nil);
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo{
     ...
     if (error == nil) {
          ...
          [SVProgressHUD showSuccessWithStatus:@"已经存入手机相册"];
     }
     else
     {
