...
@property (nonatomic , strong) NSString * selectId;//装有title的数组
...
- (void)MCPickerListView:(MCPickerListView *)MCPickerListView didSelcetedValue:(MCPickerModel *)value
{
    ...
    if (array.count ==0 && [self.delegate respondsToSelector:@selector(MCPickerView:selectId:completeTitleArray:completeStr:)]) {
        [self.delegate MCPickerView:self selectId:self.selectId completeTitleArray:self.headerDataArray completeStr:[self.headerDataArray componentsJoinedByString:@""]];
    }
