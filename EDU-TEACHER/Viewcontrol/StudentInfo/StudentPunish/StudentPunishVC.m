...
@interface StudentPunishVC ()<UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, strong) UITableView   *tableView;
@property (nonatomic, strong) NSMutableArray   *listDataArray;
@end


@implementation StudentPunishVC{
    int page;
}
...
- (void)rightBarClick {
    AddStudentPunishVC *jumpVC=[AddStudentPunishVC new];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    StudentPunishModel *model=_listDataArray[indexPath.row];
    NSString *cellIde=[NSString stringWithFormat:@"listCell%@",model.id];
    
    StudentPunishCell *cell=[tableView dequeueReusableCellWithIdentifier:cellIde];
    if (cell==nil) {
        cell=[[StudentPunishCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIde];
    }
...
-(void)getNetworkData:(int)page{
    ...
    NSString *postUrl=[NSString stringWithFormat:@"%@%@",jbad.url,@"queryStudentCfList"];
    ...
    [manager LTPost:postUrl param:paramDic success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if([[resultDic objectForKey:@"Code"] isEqualToString:@"1"]){
            ...
            for (NSDictionary *item in resultArray) {
                ...
                StudentPunishModel *model=[[StudentPunishModel alloc] initWithDic:item];
