...
@interface StudentAttentVC ()<UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, strong) UITableView   *tableView;
@property (nonatomic, strong) NSMutableArray   *listDataArray;
@end
 

@implementation StudentAttentVC{
    int page;
}
...
- (void)rightBarClick {
    AddStudentAttentVC *jumpVC=[AddStudentAttentVC new];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    StudentAttentModel *model=_listDataArray[indexPath.row];
    NSString *cellIde=[NSString stringWithFormat:@"listCell%@",model.id];
    
    StudentAttentCell *cell=[tableView dequeueReusableCellWithIdentifier:cellIde];
    if (cell==nil) {
        cell=[[StudentAttentCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIde];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    StudentAttentModel *model=_listDataArray[indexPath.row];
    StudentAttentDetailVC *jumpVC=[StudentAttentDetailVC new];
...
-(void)getNetworkData:(int)page{
    ...
    NSString *postUrl=[NSString stringWithFormat:@"%@%@",jbad.url,@"queryStudentXlList"];
    ...
    [manager LTPost:postUrl param:paramDic success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if([[resultDic objectForKey:@"Code"] isEqualToString:@"1"]){
            ...
            for (NSDictionary *item in resultArray) {
                ...
                StudentAttentModel *model=[[StudentAttentModel alloc] initWithDic:item];
