...
@interface StudentHelpVC ()<UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, strong) UITableView   *tableView;
@property (nonatomic, strong) NSMutableArray   *listDataArray;
@end


@implementation StudentHelpVC{
    int page;
}
...
- (void)rightBarClick {
    AddStudentHelpVC *jumpVC=[AddStudentHelpVC new];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    StudentHelpModel *model=_listDataArray[indexPath.row];
    NSString *cellIde=[NSString stringWithFormat:@"listCell%@",model.id];
    
    StudentHelpCell *cell=[tableView dequeueReusableCellWithIdentifier:cellIde];
    if (cell==nil) {
        cell=[[StudentHelpCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIde];
    }
...
-(void)getNetworkData:(int)page{
    ...
    NSString *postUrl=[NSString stringWithFormat:@"%@%@",jbad.url,@"queryStudentBfList"];
    ...
    [manager LTPost:postUrl param:paramDic success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if([[resultDic objectForKey:@"Code"] isEqualToString:@"1"]){
            ...
            for (NSDictionary *item in resultArray) {
                ...
                StudentHelpModel *model=[[StudentHelpModel alloc] initWithDic:item];
