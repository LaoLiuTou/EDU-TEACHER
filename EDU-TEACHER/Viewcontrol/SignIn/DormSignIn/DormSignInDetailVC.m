...
@interface DormSignInDetailVC ()<GKPageScrollViewDelegate, WMPageControllerDataSource, WMPageControllerDelegate, GKWBPageViewControllDelegate,UISearchBarDelegate>
...
@property (nonatomic, strong) DormSignInModel *model;

@property (nonatomic, strong) LTSearchBar *searchBar;
@end

@implementation DormSignInDetailVC{
    int viewHeight;
}
...
- (UIView *)headerViewInPageScrollView:(GKPageScrollView *)pageScrollView {
    ...
    DormSignInDetailView *dormSignInDetailView = [[DormSignInDetailView alloc] init];
    viewHeight=[dormSignInDetailView initModel:self.model];
    dormSignInDetailView.frame=CGRectMake(0, 50, kWidth, viewHeight);
    ...
    [_detailView addSubview:dormSignInDetailView];
...
-(void)getNetworkData:(NSString *) refresh{
    ...
    NSString *postUrl=[NSString stringWithFormat:@"%@%@",jbad.url,@"querySsqSignDetail"];
    ...
    [manager LTPost:postUrl param:paramDic success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if([[resultDic objectForKey:@"Code"] isEqualToString:@"1"]){
            ...
            DormSignInModel *model=[[DormSignInModel alloc] initWithDic:[resultDic objectForKey:@"Result"]];
            self.model=model;
