...
@interface OtherSignInDetailVC ()<GKPageScrollViewDelegate, WMPageControllerDataSource, WMPageControllerDelegate, GKWBPageViewControllDelegate,UISearchBarDelegate>
...
@property (nonatomic, strong) OtherSignInModel *model;

@property (nonatomic, strong) LTSearchBar *searchBar;
@end

@implementation OtherSignInDetailVC{
    int viewHeight;
}
...
- (UIView *)headerViewInPageScrollView:(GKPageScrollView *)pageScrollView {
    ...
    OtherSignInDetailView *otherSignInDetailView = [[OtherSignInDetailView alloc] init];
    viewHeight=[otherSignInDetailView initModel:self.model];
    otherSignInDetailView.frame=CGRectMake(0, 50, kWidth, viewHeight);
    ...
    [_detailView addSubview:otherSignInDetailView];
...
-(void)getNetworkData:(NSString *) refresh{
    ...
    NSString *postUrl=[NSString stringWithFormat:@"%@%@",jbad.url,@"queryJqSignDetail"];
    ...
    [manager LTPost:postUrl param:paramDic success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if([[resultDic objectForKey:@"Code"] isEqualToString:@"1"]){
            ...
            OtherSignInModel *model=[[OtherSignInModel alloc] initWithDic:[resultDic objectForKey:@"Result"]];
            self.model=model;
