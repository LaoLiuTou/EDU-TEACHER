...
@interface ActivitySignInDetailVC ()<GKPageScrollViewDelegate, WMPageControllerDataSource, WMPageControllerDelegate, GKWBPageViewControllDelegate,UISearchBarDelegate>
...
@property (nonatomic, strong) ActivitySignInModel *model;

@property (nonatomic, strong) LTSearchBar *searchBar;
@end

@implementation ActivitySignInDetailVC{
    int viewHeight;
}
...
- (UIView *)headerViewInPageScrollView:(GKPageScrollView *)pageScrollView {
    ...
    ActivitySignInDetailView *activitySignInDetailView = [[ActivitySignInDetailView alloc] init];
    viewHeight=[activitySignInDetailView initModel:self.model];
    activitySignInDetailView.frame=CGRectMake(0, 50, kWidth, viewHeight);
    ...
    [_detailView addSubview:activitySignInDetailView];
...
-(void)getNetworkData:(NSString *) refresh{
    ...
    NSString *postUrl=[NSString stringWithFormat:@"%@%@",jbad.url,@"queryHqSignDetail"];
    ...
    [manager LTPost:postUrl param:paramDic success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if([[resultDic objectForKey:@"Code"] isEqualToString:@"1"]){
            ...
            ActivitySignInModel *model=[[ActivitySignInModel alloc] initWithDic:[resultDic objectForKey:@"Result"]];
            self.model=model;
